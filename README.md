# Identity Development Serious Game

Unity prototype game made to help adolescents in Dutch practical schools with identity development for School courses. This project is part of the [LIFTS project](liftsproject.com) and is part of the master thesis project at the Utrecht University.

The current prototype is not a full game and merely demonstrates how an indentity development game could look like for the target audience and used to prototype ideas and generate feedback from teachers and students. The design is made to be extendable to all facets of identity development such as hobbies and family/friends and more that could be valuable in the context of school. 

<p float="left">
    <img src="Screenshots/Screenshot_20260512_111810_Identity_development_game.jpg" alt="Alt Text" width="200" height="">
    <img src="Screenshots/Screenshot_20260512_111824_Identity_development_game.jpg" alt="Alt Text" width="200" height="">
     <img src="Screenshots/Screenshot_20260512_112014_Identity_development_game.jpg" alt="Alt Text" width="200" height="">
</p>
...
The `Teacher` has limited access to the data of every `Player` only seeing what they need to perform the above tasks. The `Teacher` holds roughly the list of attributes below.
...
The `Player` will be able to access its data from the `Player` table and accesses a restricted amount of data from other players in its own leaderboard group table. The game will update itself given certain actions the player makes.
- Sync with server data about other players
...
The basic idea of the game is set in an openworld school environment where every player can do
The game includes only the beginning of 1 quest and includes 1 working minigame.
...
The GameManager reads and writes to disk to store player and server information. Will be launched with the `SchoolMap` scene and either goes to character creation if a local file is missing or loads in all previous data.
...
All models not listed here are made for this project.
- Ready player me character models (Discontinued website).
- All school models by Styloo on [itch.io](https://styloo.itch.io/classroom-asset-pack)
- Grass and pavement textures from [Blendkit](https://www.blendkit.com/)
- UI elements adapted and created from [ChatGPT](https://chatgpt.com) generated images
